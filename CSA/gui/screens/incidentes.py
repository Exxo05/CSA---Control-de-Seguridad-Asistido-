# gui/screens/incidentes.py — Gestión de incidentes v2.0
...
class IncidentesScreen(tk.Frame):
    ...
    def _build(self):
        ...
        self.ent_buscar = tk.Entry(inner_tb, font=FONT_NORMAL, relief="solid", bd=1,
                                    bg=WHITE, fg=DARK_TEXT, width=28)
        ...
        self.ent_buscar.bind("<FocusIn>",  self._clear_placeholder)
        self.ent_buscar.bind("<FocusOut>", self._restore_placeholder)
        ...
        style.map("CSA.Treeview", background=[("selected","#DBEAFE")],
        ...
        widths = {"ID":50,"Tipo":180,"Descripción":280,"Fecha":145,
                  "Ubicación":200,"Estado":110}
        for c in cols:
        ...
        for etiqueta, (bg, fg) in [
            ...
            lbl = tk.Label(leyenda, text=f"  {etiqueta}  ", font=FONT_SMALL,
                           bg=bg, fg=fg, relief="solid", bd=1, padx=4, pady=2)
    ...
    def cargar_datos(self):
        ...
        for row in registros:
            # row: (id, tipo, desc, fecha, zona, direccion, estado)
            estado = str(row[6]).strip() if len(row) > 6 else "Activo"
            ubi = f"{row[5]}  ({row[4]})" if row[5] else row[4]
            estado_visual = "✅ Finalizado" if estado == "Finalizado" else "🚨 Activo"
            self._todos.append({
                "id": row[0], "tipo": row[1], "desc": row[2],
                "fecha": row[3], "ubi": ubi, "estado": estado,
                "estado_v": estado_visual,
                "tag": "finalizado" if estado == "Finalizado" else _tag_desde_tipo(row[1])
            })
        self._filtrar()
    ...
    def _filtrar(self):
        ...
        for r in self._todos:
            ...
            # Filtro búsqueda
            if query and not any(query in str(v).lower() for v in [r["tipo"],r["desc"],r["ubi"]]):
                continue
            self.tree.insert("", "end", iid=str(r["id"]),
                              values=(r["id"], r["tipo"], r["desc"][:60]+"…" if len(r["desc"])>60 else r["desc"],
                                      r["fecha"], r["ubi"], r["estado_v"]),
                              tags=(r["tag"],))
            mostrados += 1

        total = len(self._todos)
        self.lbl_conteo.config(text=f"{mostrados} de {total} incidentes")
    ...
    def _editar(self):
        ...
        # vals: (id, tipo, desc, fecha, ubi, estado_v)
        EditorIncidente(self, vals[0], vals[1], vals[2], vals[4], on_save=self.cargar_datos)
...
class EditorIncidente(tk.Toplevel):
    def __init__(self, parent, inc_id, tipo, desc, ubi, on_save):
        ...
        tk.Label(self, text=f"✏️  Editar Incidente #{inc_id}",
                 font=("Segoe UI", 13, "bold"), bg=WHITE, fg=POLICE_BLUE).pack(anchor="w", pady=(0,12))

        def field(lbl, val, height=None):
            tk.Label(self, text=lbl, font=("Segoe UI",9), bg=WHITE, fg=GRAY_TEXT).pack(anchor="w")
            if height:
                w = tk.Text(self, height=height, font=FONT_NORMAL, relief="solid", bd=1, bg=WHITE)
                w.insert("1.0", val)
            else:
                w = tk.Entry(self, font=FONT_NORMAL, relief="solid", bd=1, bg=WHITE)
                w.insert(0, val)
