# gui/screens/recomendacion.py — Asistente de despliegue v2.0
...
def _gravedad_desde_tipo(tipo: str):
    t = str(tipo).upper()
...
class RecomendacionScreen(tk.Frame):
    ...
    def _build(self):
        ...
        if not incidentes:
            tk.Label(left,
                     ...
                     font=("Segoe UI", 12), fg="#059669", bg=WHITE,
                     pady=40).pack()
            return
        ...
        self.combo_inc["values"] = [
            f"#{i[0]}  |  {i[1]}  —  {i[4]}" for i in incidentes
        ]
        ...
        make_button(btn_frame, "🔄  Refrescar",
                    self._build_restart, "info").pack(side="right", padx=(0,8))
    ...
    def _analizar(self, event=None):
        ...
        self.lbl_gravedad.config(
            text=f"  {emoji}  Nivel {nivel}  ",
            bg=bg, fg=fg
        )
        ...
        # Preseleccionar las primeras N sugeridas
