# gui/screens/mapa_calor.py — v3.2
...
class MapaCalorScreen(tk.Frame):
    ...
    def _on_cambio(self, **kw):
        ...
        self._debounce = self.after(1500, self._actualizar_estado)

    def _actualizar_estado(self):
        if not self._viva: return
        self._debounce = None
        hora = datetime.datetime.now().strftime("%H:%M:%S")
        self.lbl_estado.config(text=f"✅ Actualizado — {hora}", fg="#059669")
        if getattr(self, "_wv_ok", False):
            ts = int(datetime.datetime.now().timestamp())
            try:
                self._wv.load_url(f"{mapa_server.url_calor()}?t={ts}")
            except Exception:
                pass
    ...
    def _build(self):
        ...
        lbl = tk.Label(inner, text=f"🔗 {mapa_server.url_calor()}",
    ...
    def _build_panel(self):
        try:
            ...
            self._wv = tkinterweb.HtmlFrame(
                self.frame_mapa, messages_enabled=False)
            self._wv.pack(fill="both", expand=True)
            self._wv.load_url(mapa_server.url_calor())
            self._wv_ok = True
            return
        except Exception:
            pass
        ...
        c.pack(expand=True)
        tk.Label(c, text="🔥", font=("Segoe UI", 52), bg=WHITE).pack(pady=(30, 8))
        tk.Label(c, text="Mapa de Calor — Auto-refresco activo",
                 font=("Segoe UI", 15, "bold"), bg=WHITE, fg=POLICE_BLUE).pack()
        tk.Label(c,
                 text="Se regenera automáticamente al registrar incidentes.\n"
        ...
        make_button(c, "🌐  Abrir mapa de calor",
                    self._abrir, "danger").pack(pady=8)
